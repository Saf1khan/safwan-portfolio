...
// 16-point starburst compass SVG matching the reference site
const StarburstIcon = ({ rotation }: { rotation: MotionValue<string> }) => (
  <div className="arrow lg:scale-100 scale-[0.6] inline-flex items-center shrink-0">
    <motion.svg
      className="h-9 w-9 ml-2 text-current"
...
// Single Phrase item matching reference DOM structure exactly
const PhraseItem = ({
  text,
  rotation,
}: {
  ...
  <div className="flex items-center lg:gap-4 gap-2 flex-nowrap flex-shrink-0 w-fit justify-center">
    {text}
    <StarburstIcon rotation={rotation} />
  </div>
);

// Triplicate Marquee Track matching reference HTML layout (left copy, center, right copy)
const MarqueeTrackGroup = ({
  transformX,
  rotation,
}: {
  ...
  <div className="relative flex gap-6 items-start w-max">
    {/* Left absolute track (right-[100%]) */}
    <motion.div
      style={{ transform: transformX, willChange: "transform" }}
      className="flex gap-6 flex-row mr-14 absolute w-full whitespace-nowrap right-[100%] top-0 items-center shrink-0"
    >
      {marqueeItems.map((text, i) => (
        <PhraseItem key={`left-${i}`} text={text} rotation={rotation} />
      ))}
    </motion.div>

    {/* Center in-flow track */}
    <motion.div
      style={{ transform: transformX, willChange: "transform" }}
      className="flex w-full gap-4 flex-row whitespace-nowrap items-center shrink-0"
    >
      {marqueeItems.map((text, i) => (
        <PhraseItem key={`center-${i}`} text={text} rotation={rotation} />
      ))}
    </motion.div>

    {/* Right absolute track (left-[100%]) */}
    <motion.div
      style={{ transform: transformX, willChange: "transform" }}
      className="flex gap-6 flex-row ml-5 absolute w-full whitespace-nowrap left-[100%] top-0 items-center shrink-0"
    >
      {marqueeItems.map((text, i) => (
        <PhraseItem key={`right-${i}`} text={text} rotation={rotation} />
      ))}
    </motion.div>
  </div>
);
...
export const SlidingBars = () => {
  ...
  // Position motion values for Bar 1 and Bar 2 (in percentage units -100 to 0)
  const baseX1 = useMotionValue(-15.6481);
  const baseX2 = useMotionValue(0.7516);
  const baseRotation = useMotionValue(288.774);

  // Track page scroll velocity to dynamically accelerate marquee & starburst spin on scroll
  ...
  // Base cruising speeds (percentage per second)
  const baseSpeed = 2.4; // % per second
  const rotSpeed = 36; // degrees per second

  useAnimationFrame((_, delta) => {
    const dt = delta / 1000;
    // Current scroll velocity factor (normalized)
    const vel = smoothVelocity.get();
    const velocityMultiplier = Math.min(Math.max(vel / 200, -10), 10);

    // Bar 1 glides to the left (negative direction), accelerates with scroll down
    const move1 = -(baseSpeed * dt) - (velocityMultiplier * dt * 3.5);
    baseX1.set(wrap(-100, 0, baseX1.get() + move1));

    // Bar 2 glides to the right (positive direction), reacts with opposite momentum
    const move2 = (baseSpeed * dt) + (velocityMultiplier * dt * 3.5);
    baseX2.set(wrap(-100, 0, baseX2.get() + move2));

    // Starburst icon spins clockwise continuously and spins faster with scroll velocity
    const rotDelta = (rotSpeed * dt) + (Math.abs(velocityMultiplier) * dt * 60);
    baseRotation.set((baseRotation.get() + rotDelta) % 360);
  });

  // Format transforms matching the reference inspected DOM format
  ...
  const rotationTransform = useTransform(
    baseRotation,
    (val) => `translate3d(0px, 0px, 0px) rotate(${val.toFixed(3)}deg)`
  );

  const sharedRibbonClass =
    "overflow-hidden font-cabinetGrotesk whitespace-nowrap text-main font-bold text-sm lg:text-3xl " +
    "z-20 absolute top-[50%] -translate-y-1/2 transform translate-x-[50%] marquee right-[50%] font-righteous " +
    "lg:py-5 py-3 bg-sec select-none pointer-events-none " +
    "w-[150vw] min-w-[150vw]";

  return (
    <section
      ref={sectionRef}
      className="SlidingBars overflow-hidden md:h-[50vh] h-[25vh] mb-8 relative w-full"
    >
      {/* Bar 1: Clockwise tilt (lg:rotate-6 rotate-12) */}
      <div className={`${sharedRibbonClass} lg:rotate-6 rotate-12`}>
        <MarqueeTrackGroup transformX={transform1} rotation={rotationTransform} />
      </div>

      {/* Bar 2: Counter-clockwise tilt (lg:-rotate-6 -rotate-12) */}
      <div className={`${sharedRibbonClass} lg:-rotate-6 -rotate-12`}>
        <MarqueeTrackGroup transformX={transform2} rotation={rotationTransform} />
      </div>
    </section>
  );
};
